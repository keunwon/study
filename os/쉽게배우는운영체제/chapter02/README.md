...
- 시스템 버스와 CPU 내부 버스
    - 시스템 버스는 메모리와 주변 장치를 연결하는 버스로 FSB(Front-Side Bus)
...
- 명령어 레지스터(IR)
    - 현재 실행 중인 명령어를 저장
    - 제어장치는 명령어 레지스터에 있는 명령을 해석 후 외부 장치에 적절한 제어 신호를 보냄
- 메모리 주소 레지스터(MAR)
...
### 메모리 보호
- 사용자 영역의 작업이 운영체제 영역이나 다른 프로그램 영역에 침범하지 못하도록 막는 것
...
- 데이터 위협, 제어 위험, 구조적 위험의 문제가 있음 (하이퍼 스레드를 이용하여 문제 해결)
- 한 클록에 한 명령어만 처리 함
### 슈퍼스칼라 기법
