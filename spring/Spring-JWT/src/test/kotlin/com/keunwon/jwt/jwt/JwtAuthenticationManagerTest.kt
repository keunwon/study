...
internal class JwtAuthenticationManagerTest {
    ...
    @MockK lateinit var customUserDetailsService: CustomUserDetailsService<User, Long>
    ...
    fun `사용자 로그인 성공`() {
        // given
        every { customUserDetailsService.findByAuthentication(authenticationToken) } returns
    ...
    fun `사용자가 존재하지 않으면 'InternalAuthenticationServiceException' 발생`() {
        // given
        every { customUserDetailsService.findByAuthentication(authenticationToken) } returns null
