...
## 2.12. 장기 작업 타이머
- 장기 작업 타이머는 측정하고 있는 이벤트가 실행되는 동안 계속 시간을 측정할 수 있는 특별한 타이머 (작업이 완료되기 전에는 경과 시간을 기록하지 않음)
## 2.13. 미터 타입 선정
- 셀수 있는 것을 게이지로 측정하지 말고 시간으로 측정할 수 있는 것을 세려고 하면 하면 안됨
- 매번 2분을 초과하는 요청을 측정할 때는 LongTaskTimer를 선택
- 인플라이어트 요청(=진행 중인 요청), 특히 실패 가능성이 있고 예상 시간이 수 분, 수 시간 단위로 늘어날 위험이 있는 요청도 LongTaskTimer로 측정
## 2.14. 비용 제어
- 측정 애플리케이션이 늘어날수록 메트릭 비용도 증가
- 카디널리티의 경계는 가능한 모든 키-값 태그 좋바을 고려해 신중하게 결정해야 함
	- 넷플릭스에서 일반적으로 제시하는 선은 시계열이 100만을 넘지 않을 정도로 카디널리티를 유지 (대부분의 조직들은 이 정도까지 필요하지 않음)
- 비용을 과도하게 최적화하려는 충동은 자제해야 함
	- '0'이 게시되는 것을 막으려고 할 필요는 없음
## 2.15. 조율된 누락
- 서버리스 함수: 서버리스 함수의 실행 시간을 측정할 때는 함수가 시작되기까지 걸리는 시간을 기록하지 않음 
- 일시 정지: GC로 인한 JVM 중단, 데이터베이스 재인덱싱으로 인한 일지적 응답 불가, 캐시 버퍼를 디스크로 플러시하는 등
- 부하 테스트: 서비스가 실제로 포화되기 전에 자신의 스레드 풀에 작업을 백업
## 2.17. 미터 필터
## 2.18. 플랫폼과 애플리케이션 메트릭 분리
- 플랫폼 엔지니어
	- 플랫폼 엔지니어는 조직 전체에 존재하는 모든 마이크로서비스를 동일한 방식으로 모니터링하기 원함
	- 특정 비지니스 기능과 관련이 없는 일반적인 지표를 나타내므로 시간이 지나더라도 크게 변경되지 않음
- 프로덕트 엔지니어
	- 프로덕트 엔지니어는 프로덕션과 관련된 마이크로서비스만 모니터링 함
	- 개별 인스턴스를 더욱 차별화하는 공통 태그를 추가하기를 원함
## 2.19. 모니터링 시스템에 따른 메트릭 분할
- 현재 클라우드 워치를 사용하려는 이유가 프로메테우스 모니터링 시스템이 할 일을 다하고 있는지 검증하는 용도인 경우
	- 플랫폼팀은 자산 인벤토리 배포 상태를 정기적으로 조회
	- 마이크로미터는 프로메테우스가 스크래핑을 시도하는 횟수를 기록(스크래핑되지 않는 애플리케이션은 이 횟수가 0이거나 비어 있으면 누락된 애플리케이션을 식별할 수 있음)
## 2.20. 미터 바인더
- 마이크로미터는 여러 미터를 한 곳에 캡슐하도록 고안된 'MeterBinde' 인터페이스를 제공
